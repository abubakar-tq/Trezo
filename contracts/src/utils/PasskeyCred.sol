...
library PassKeyDemo {
    ...
    bytes32 private constant _DEFAULT_RP_HASH =
        0x638841ea13dd17405349cb4795e780a1105648d79c51e6671af0a66d7597f945;
    ...
    /// @notice Return the fixture RP hash used to build authenticatorData in tests.
    function getPasskeyRpHash(uint256 index) external pure returns (bytes32 rpHash) {
        rpHash = _getPasskeyRpHash(index);
    }
    ...
    function _getPasskeyRpHash(uint256 index) private pure returns (bytes32 rpHash) {
        if (index < _PASSKEY_COUNT) return _DEFAULT_RP_HASH;
        revert("PassKeyDemo: index out of bounds");
    }
}
