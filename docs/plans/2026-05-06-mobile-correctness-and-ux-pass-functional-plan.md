# Mobile Correctness & UX Polish Pass — Functional Implementation Plan
...
**Companion plan:** `docs/plans/2026-05-06-mobile-correctness-and-ux-pass-plan.md` is the test-first variant of this work. This plan is the same scope without the custom-test scaffolding (the mobile workspace has no Jest runner per CLAUDE.md, so writing/running test files added overhead without payoff). Functional verification is via TypeScript + lint + manual smoke against the local Anvil + bundler stack.
...
> **Status:** Already implemented in commit `7c60407de feat(portfolio): add TokenDiscoveryProvider + RegistryDiscoveryProvider`. Skip if the file exists; verify with the spot-check below.
...
- [x] **Step 2: Confirm `dexRegistry` shape**
...
**Notes on the design:**
- Catches `getTopAssets` failure silently — a CoinCap outage shouldn't break the home screen; it just makes USD values resolve to `null` and surfaces via the "USD unavailable" hint added in A1.5.
- Symbol-only matching is sufficient for the registry tokens (ETH, USDC, DAI, etc.) and matches what `MarketService` already returns. Address-based matching can be added when an indexer is integrated later.

- [ ] **Step 2: Confirm `marketService.getTopAssets` shape**

Run: `grep -n "getTopAssets\|priceUsd\|symbol" apps/mobile/src/services/MarketService.ts | head -20`

The return type should expose at least `{ symbol, priceUsd }`. If shape diverges, adapt `MarketServiceLike` to match.
...
- `getPortfolio(address, chainId, deps?)` instead of using a hardcoded `anvil` chain.
- The hardcoded `3245.32` ETH fallback is gone. Tokens without prices contribute `null`, are excluded from `totalValue`, and surface via `missingPrices`.
- Discovery and price fetching are pluggable (via `deps`) for future Moralis/indexer/CoinGecko swaps.
- Per-(chain, address) caching with 30 s TTL preserves the existing cache behavior.
...
**Why preserve `MoralisToken`:** The shape is consumed by other components (e.g., token list). Swapping the producer (`PortfolioService`) without changing the consumer-facing type keeps the diff small.
...
If errors mention `aaAccount?.chainId` typing, cast as in `DexScreen.tsx:98-102`.
...
{missingPrices && missingPrices.length > 0 ? (
  <Text style={[styles.missingPriceHint, { color: theme.colors.textMuted }]}>
    USD unavailable for {missingPrices.length} token{missingPrices.length === 1 ? "" : "s"}
  </Text>
) : null}
```

- [ ] **Step 4: Add the style entry**
...
- [ ] **Step 5: Pass `missingPrices` from `HomeScreen` into `BalanceCard`**
...
**Why `NUMERIC(78, 0)`:** Matches the precision of a 256-bit unsigned integer (`type(uint256).max` is 78 digits). Supabase serializes these as strings, preserving the bigint without floating-point loss.
...
(`string | null` because NUMERIC(78,0) columns serialize as strings to preserve bigint precision.)
...
2. Modify the `detailRows` `useMemo` (lines 66-110). Do NOT rewrite the existing element array. Instead, replace its `return [...]` line with `const rows = [...];` (i.e., assign to a `const`), then *after* the array literal closes, before the closing `}, [row]);`, insert:
...
The existing element array is preserved unchanged — only the variable binding (`return [...]` → `const rows = [...];`) and the appended logic before `return rows;` change.
...
**Why the `ranRef`:** The hook is mounted on every nav transition; we only want one backfill per (logged-in user) session.
...
- [ ] **Step 2: Replace the loader with a re-runnable callback + cancel-safe effect**
...
(Note: `runChecks` has internal short-circuit on `user?.id` change because closures capture the latest state; the `cancelled` flag mainly guards against late re-renders post-unmount.)
...
In the render, derive the title and body from state + reason:
...
**Design notes:**
- Pure function with no I/O — safe to call from any layer.
- Message-substring matching is intentionally permissive — it catches viem/ethers/bundler errors with similar phrasing without bespoke per-source parsers.
- The `simulation_revert` kind is reserved for a future task that parses simulator output specifically; nothing currently produces it.

- [ ] **Step 2: Spot-check classification with a sanity REPL** (optional)

If you want to validate the classifier ad-hoc without running the full app, add a temporary `console.log(classify(new Error("...")));` inside a known callsite during the C2.6 wire-up. Remove before commit.
...
- Only network-flavored errors are retried; non-retryable errors (e.g., revert) abort early.
- Linear backoff (150 ms × attempt) — fast enough for an RPC blip, slow enough to avoid hammering.
...
**Decision rationale:** Re-prepare silently when drift is within slippage; only surface the warning when the user would actually receive worse than they accepted.
...
- **Differences from the test-first variant.** This plan drops the `__tests__` files and the "write failing test → run test → write impl → verify pass" loop because the mobile workspace has no Jest runner. Verification gates are TypeScript + ESLint + manual smoke against Anvil. All implementation code, JSX, SQL, and type signatures are identical to the test-first variant.
...
- **Ambiguity handling.** Where a callsite or signature couldn't be fully verified from the spec alone (e.g., `getRpcUrl` chain-id arg, `markConfirmed` receipt shape), the task includes an explicit verification step (`grep` or `Run`) and an "adjust if necessary" instruction.
