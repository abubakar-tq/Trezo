...
interface SignTypedSheetProps {
  req: { id: string; origin: string; chainId: number; typedData: unknown };
}

export function SignTypedSheet({ req }: SignTypedSheetProps) {
  ...
  async function sign() {
    ...
    try {
      ...
      // 3. Guard: reject if typed data domain chainId doesn't match session chain
      // (prevents dApp tricking user into signing a mainnet digest while on testnet)
      const domainChainId = (req.typedData as { domain?: { chainId?: unknown } })?.domain?.chainId;
      if (domainChainId !== undefined && Number(domainChainId) !== req.chainId) {
        const mismatchMsg = `Typed data domain chainId (${domainChainId}) doesn't match connected chain (${req.chainId}). Rejected.`;
        await rejectApproval(req.id, mismatchMsg);
        setErr(mismatchMsg);
        return;
      }

      // 4. EIP-712 digest → passkey sign → EIP-1271 envelope
      const digest = hashTypedData(req.typedData as Parameters<typeof hashTypedData>[0]);
    ...
    <Sheet>
      ...
      <div className="flex items-center justify-between mb-[12px]">
        <div className="flex items-center gap-[9px]">
          <Logo size={26} />
          <b className="text-[15px] font-semibold tracking-tight">Sign typed data</b>
        </div>
      ...
      <Card className="mb-[12px]">
        ...
        <pre
          className="font-mono text-[11px] leading-[1.6] whitespace-pre-wrap break-all overflow-auto"
          style={{ maxHeight: 200, color: "var(--text)" }}
        >
