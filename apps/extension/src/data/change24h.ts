/**
 * change24h.ts — pure portfolio 24h change helper
 *
 * Ported from apps/mobile/src/features/home/utils/portfolio24h.ts
 * No React-Native or mobile deps. Suitable for extension popup + service worker.
 *
 * Spec §5.1:
 *   value24hAgo = value / (1 + pct/100)
 *   delta       = Σvalue − Σvalue24hAgo
 *   pct         = delta / Σvalue24hAgo × 100
 *
 * Returns null when no holding has a known 24h% (never fabricates 0%).
 */
...
export interface Change24hResult {
  /** Absolute portfolio USD change over the past 24 h. */
  delta: number;
  /** Portfolio % change over the past 24 h (deposit-neutral denominator). */
  pct: number;
}
...
export function computeTotalChange24h(
  ...
  if (totalValue24hAgo === 0) return null;
