...
const RootNavigation = () => {
  ...
  console.log('🔄 [RootNavigation] Rendering, isLoggedIn:', isLoggedIn, 'showingSplash:', showingSplash);
  ...
  const getRedirectTarget = () => {
    console.log('🎯 [RootNavigation] Getting redirect target, isLoggedIn:', initialAuthState);
    if (!initialAuthState) {
      // User not logged in - go to Auth stack, will show Introduction
      console.log('👤 [RootNavigation] User not logged in, showing AuthNavigation');
    ...
    console.log('🔒 [RootNavigation] User logged in, redirecting to DeviceVerification');
  ...
  const redirectTarget = getRedirectTarget();
	...
	return (
		<NavigationContainer 
			...
			onStateChange={(state) => console.log('📍 [Navigation] State changed:', JSON.stringify(state?.routes[state.index], null, 2))}
		>
			<Stack.Navigator
				initialRouteName={showingSplash ? "AppSplash" : (initialAuthState ? "DeviceVerification" : "AuthNavigation")}
				screenOptions={{
					headerShown: false,
			...
			>
				{showingSplash ? (
					<Stack.Screen
						...
						initialParams={{ redirectTo: redirectTarget ? { name: redirectTarget } : undefined }}
						listeners={{
					...
					<>
						{initialAuthState && (
							<Stack.Screen 
								...
								listeners={{
									focus: () => console.log('👀 [Navigation] DeviceVerification focused'),
								}}
							/>
						)}
						<Stack.Screen 
							...
							listeners={{
								focus: () => console.log('👀 [Navigation] AuthNavigation focused'),
							}}
						/>
						<Stack.Screen 
							name="TabNavigation" 
							...
							listeners={{
								focus: () => console.log('👀 [Navigation] TabNavigation focused'),
							}}
						/>
						<Stack.Screen 
