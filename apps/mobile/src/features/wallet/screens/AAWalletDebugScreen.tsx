...
export default function AAWalletDebugScreen() {
  ...
  const styles = useMemo(
    () => (colors ? createStyles(colors) : null),
    [colors],
  );
  ...
  const deploymentStatus = useWalletStore(
    (state) => state?.accountDeploymentStatus,
  );
  ...
  const updateTest = useCallback(
    (index: number, updates: Partial<TestResult>) => {
      setTests((prev) =>
    ...
    try {
      ...
      if (!isReady) {
        updateTest(1, {
          status: "error",
          message:
            "⚠️ Contracts not deployed yet - addresses are placeholder 0x000...",
          data: contracts,
        });
      } else if (!isValid) {
        updateTest(1, {
  ...
  const testCheckDeployment = useCallback(async () => {
    ...
    try {
      const aaService = getAAWalletService();
      const isDeployed = await aaService.isAccountDeployed(
  ...
  const testBundler = useCallback(async () => {
    ...
    try {
      ...
      updateTest(6, {
        status: "error",
        message:
          error instanceof Error ? error.message : "Bundler connection failed",
      });
    }
  }, [updateTest]);
  ...
  const handleViewDatabase = useCallback(async () => {
    ...
    try {
      ...
      const guardians = wallet
        ? await supabaseService.getGuardians(wallet.id)
        : [];
      const transactions = wallet
        ? await supabaseService.getTransactions(wallet.id, 10)
        : [];
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.stateCard}>
            ...
            <View style={styles.stateRow}>
              ...
              <Text style={styles.stateValue}>
                {aaAccount
                  ? `${aaAccount.predictedAddress.slice(0, 8)}...${aaAccount.predictedAddress.slice(-6)}`
                  : "Not created"}
              </Text>
            </View>
          ...
          {tests.map((test, index) => (
            ...
            >
              ...
              {test.data && test.status === "error" && (
                <View style={styles.testData}>
                  <Text
                    style={[styles.testDataText, { color: colors.danger }]}
                    numberOfLines={3}
                  >
                    {(() => {
                      try {
        ...
        <View style={styles.section}>
          ...
          <View style={styles.instructionCard}>
            ...
            <Text style={styles.instructionText}>
              Once contracts are deployed, use &quot;Deploy Account&quot; to
              test the full flow
            </Text>
          </View>
        </View>
