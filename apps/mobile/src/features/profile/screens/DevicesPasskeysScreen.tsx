...
const DevicesPasskeysScreen: React.FC = () => {
  ...
  const submitRemovalAction = useCallback(
    async (type: "schedule" | "cancel" | "execute", device: WalletDevice) => {
      if (!user?.id || !walletAddress) return;
      ...
      const built =
        type === "schedule"
          ? await PasskeyAccountService.buildScheduleRemovePasskeyUserOp(payload)
          : type === "cancel"
            ? await PasskeyAccountService.buildCancelRemovePasskeyUserOp(payload)
            : await PasskeyAccountService.buildExecuteRemovePasskeyUserOp(payload);
  ...
  const handleScheduleRemoval = useCallback(
    async (device: WalletDevice) => {
      ...
      setError(null);
      try {
        await submitRemovalAction("schedule", device);
        await loadData();
      } catch (err) {
        setError(err instanceof Error ? err.message : "Failed to schedule removal");
      } finally {
        setBusy(false);
      }
    },
  ...
  const handleCancelRemoval = useCallback(
    async (device: WalletDevice) => {
      ...
      setError(null);
      try {
        await submitRemovalAction("cancel", device);
        await loadData();
      } catch (err) {
        setError(err instanceof Error ? err.message : "Failed to cancel removal");
      } finally {
        setBusy(false);
      }
    },
    [loadData, submitRemovalAction],
  );

  const handleExecuteRemoval = useCallback(
    async (device: WalletDevice) => {
      if (device.removal_execute_after && new Date(device.removal_execute_after).getTime() > Date.now()) {
        Alert.alert("Removal not ready", "This passkey removal is still timelocked.");
        return;
      }
      setBusy(true);
      setError(null);
      try {
        await submitRemovalAction("execute", device);
        await loadData();
      } catch (err) {
        setError(err instanceof Error ? err.message : "Failed to execute removal");
      } finally {
        setBusy(false);
      }
    },
            ...
            devices.map((device) => (
              <View key={device.id} style={styles.deviceRow}>
                ...
                {device.status === "active" ? (
                  <TouchableOpacity style={styles.actionButton} onPress={() => void handleScheduleRemoval(device)}>
                    <Text style={styles.actionLabel}>Schedule remove</Text>
                  </TouchableOpacity>
                ) : null}
                {device.status === "pending_removal" ? (
                  <View style={styles.actionsRow}>
                    <TouchableOpacity style={styles.actionButton} onPress={() => void handleCancelRemoval(device)}>
                      <Text style={styles.actionLabel}>Cancel</Text>
                    </TouchableOpacity>
                    {device.removal_execute_after &&
                    new Date(device.removal_execute_after).getTime() <= Date.now() ? (
                      <TouchableOpacity style={styles.actionButton} onPress={() => void handleExecuteRemoval(device)}>
                        <Text style={styles.actionLabel}>Execute</Text>
                      </TouchableOpacity>
                    ) : (
                      <Text style={styles.muted}>Execute becomes available after the delay expires.</Text>
                    )}
                  </View>
                ) : null}
              </View>
